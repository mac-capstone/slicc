...
export default function AddExpense() {
  ...
  useEffect(() => {
    if (tempExpense?.name !== undefined) {
      setExpenseName(tempExpense.name);
    }
  }, [tempExpense?.id, tempExpense?.name]);
...
const TempItemCard = React.memo(function TempItemCard({
  ...
  const baseAmount = !item.isTip && rate > 0 ? gross / (1 + rate / 100) : gross;
  const taxAmount = !item.isTip && rate > 0 ? gross - baseAmount : 0;
              ...
              item.amount !== undefined && (
                <View className="mt-1 flex flex-row justify-between">
                  <Text className="text-xs text-neutral-400">
                    Base: ${baseAmount.toFixed(2)} + Tax ({item.taxRate}%): $
                    {taxAmount.toFixed(2)}
                  </Text>
                </View>
              )}

...
/** Vivid red (Tailwind red-400) so cycle edges read clearly on dark bg */
const CYCLE_HIGHLIGHT_COLOR = '#F87171';
...
export function DebtGraphSvgEdges({
  ...
  const computed = React.useMemo(() => {
    ...
    edges.forEach((edge, index) => {
      ...
      const strokeBoost = isCycle ? 1.5 : 0;
      const swFinal = Math.min(sw + strokeBoost, MAX_STROKE + 2);
      geoPairs.push({ geo, color, sw: swFinal, isCycle });
    });
      ...
      {computed.geoPairs.map(({ geo, color, sw, isCycle }, i) => (
        <G key={`edge-${i}`}>
          <Path
            ...
            strokeWidth={sw}
            fill="none"
            opacity={isCycle ? 0.5 : 0.15}
          />
          <FlowingEdge
            d={geo.d}
            color={color}
            strokeWidth={sw}
            opacity={isCycle ? 0.98 : 0.65}
          />
        </G>
      ))}
    </G>

...
/** Same visibility rule as {@link buildDebtEdges} (creator, payer, or participant). */
export function isExpenseVisibleToViewer(
  expense: ExpenseResponse,
  viewerUserId: UserIdT
): boolean {
  return (
    expense.createdBy === viewerUserId ||
    getPayerId(expense) === viewerUserId ||
    expense.people.some((p) => p.id === viewerUserId)
  );
}
...
export function buildDebtEdges(
  ...
  const visible = expenses.filter((e) =>
    isExpenseVisibleToViewer(e, viewerUserId)
  );
